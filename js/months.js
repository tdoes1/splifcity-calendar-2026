...
// US Holidays for 2026 (monthIndex is 0-based: Jan=0, Feb=1, ... Dec=11)
const holidays = {
  "0-1":  "New Year's Day",
...
function buildGrid(monthIndex) {
  ...
  // build month-specific holiday list
  const monthHolidays = [];
  for (let d = 1; d <= daysInMonth; d++) {
    const key = `${monthIndex}-${d}`;
    const holidayName = holidays[key];
...
function openReminder(el) {
  ...
  // If user has no note yet but it's a holiday, prefill with holiday name
